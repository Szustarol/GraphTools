...
void graph_edge::draw(QOpenGLShaderProgram & shader, QOpenGLWidget * parent, QMatrix4x4 * view, QMatrix4x4 * projection, bool weighted){
    ...
    start_point.setX(start_point.x() + screen_offset_x);
    start_point.setY(start_point.y() + screen_offset_y);
    ...
    vao.release();
    shader.release();
    QVector4D tr = QVector4D((3*begin_pos+end_pos)/4 + QVector2D(screen_offset_x, screen_offset_y), 1.0f, 1.0f);
    ...
    QVector4D tr_2 = QVector4D((begin_pos+end_pos)/2 + QVector2D(screen_offset_x, screen_offset_y), 1.0f, 1.0f);
